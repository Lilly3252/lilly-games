import { BoardData, BoardSpace, DiceRollResult } from "#type/monopoly.js";
import { CollectorFilter, GuildTextBasedChannel, Message, MessageCollector } from "discord.js";
...
export class Monopoly {
	 ...
	 */
	constructor() {
		this.messageCollector = this.textChannel.createMessageCollector({ filter: this.filterByCurrentPlayer });
		this.players = [];
	}
	/**
	 ...
	 */
	private async throwDice(): Promise<DiceRollResult> {
	...
	public async MakeDiceRoll(player: Player): Promise<void> {
		const rollResult: DiceRollResult = await this.throwDice();
 ...
 * If the player lands on a property owned by another player, rent is paid and transferred.
 * If the player has enough money, they can buy the property; otherwise, an auction is started.
 ...
 */
	 ...
	 */
	private async startAuction(propertyName: string, propertyCost: number): Promise<void> {
		const currentPlayer = this.players[this.currentPlayerIndex];
		const property = this.propertyMap[propertyName];
	 ...
	 */
	public async handleChanceCard(player: Player) {
	 ...
	 */
	public async handleCommunityCard(player: Player) {
