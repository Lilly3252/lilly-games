import { BoardData, ChanceCard } from "#type/index.js";
...
export class ChanceCardHandler {
	 ...
	 */
	public async handleAdvance(player: Player, amount:string |number ) {
		if (isNumber(amount)) {
			const amountPosition = this.board[amount].name;
			player.move({ number: Number(amountPosition) });
			if (player.boardPosition && player.boardPosition.name > amountPosition) {
				player.receiveMoney({ amount: 200 } );
			}
		} else if (amount === "utility") {
			const closestUtilityIndex = await this.findClosestUtility(player.boardPosition.name);
			if (closestUtilityIndex !== -1) {
				player.receiveMoney({ amount: 200 });
				player.move({ number: closestUtilityIndex });
			}
		} else if (amount === "railroad") {
			const closestRailroadIndex = await this.findClosestRailroad(player.boardPosition.name);
	 ...
	 */
	public handleJail(player: Player) {
		goToJail(player);
	}
	/**
