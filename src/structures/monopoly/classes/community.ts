import { CommunityCard } from "#type/index.js";
import { goToJail } from "../functions/functions ";
import communityCardsData from "./../JSON/community.json";
import { default as MonopolyPlayer, default as Player } from "./player.js";
/**
...
export class CommunityCardHandler {
	...
	public players: MonopolyPlayer[]; // Reference to the players in the Monopoly game
	 ...
	 */
	public async handleCommunityCard(player: MonopolyPlayer, card: CommunityCard): Promise<void> {
		switch (card.type) {
			case "advance":
	...
	public async handleAdvance(player: Player, amount: number | number[]) {
		await player.move({ number: Number(amount) });
	}
	...
	public async handleEarn(player: Player, amount: number) {
		await player.receiveMoney({ amount: amount });
	}
	 ...
	 */
	public async handleRepairs(player: MonopolyPlayer,house:number, hotel: number) {
